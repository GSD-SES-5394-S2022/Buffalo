...
This file collects data relevant to the Buffalo Infrastructure project and creates graphics and tables to better visualize both the current and existing conditions of the Buffalo-Cheektowaga-Niagara Falls MSA, GEOID: 15380


Create a dot-density map
Create a histogram
Create a stacked bar chart or pie chart
Create a tree map
...
hh_1person_pts <- st_sample(no_zero_hh1, 
                            size = ceiling(no_zero_hh$hh_1personE/100))
...
#Create Tree Map of Employment types
```{r}
...
Employment <- c("basic", "retail", "service")
empsdf <- data.frame(emps, names)

emp_bar_chart <- ggplot(empsdf, aes(fill=Employment, 
                   x=1, 
                   y=emps,
                   label = emps)) + 
